...
{
    events[eventhead] = *ev;
    eventhead = (eventhead+1)&(MAXEVENTS-1);  // FIX: unsequenced modification
}
...
void D_ProcessEvents (void)
{
    ...
    for ( ; eventtail != eventhead ; eventtail = (eventtail+1)&(MAXEVENTS-1) )  // FIX: unsequenced modification
    {
	ev = &events[eventtail];
	if (M_Responder (ev))
...
*/
    ...
    S_Init (sfxVolume * 8 + (sfxVolume >> 1), musicVolume * 8 + (musicVolume >> 1) );
	...
	statcopy = (void*)(size_t)atoi(myargv[p+1]);
